...
overflow-y:scroll; // to get scrollbar only for y axis
	...
	<button class="btn btn-xs btn-danger" style="margin-left: 8px; margin-bottom: 8px;" data-toggle="modal" data-target="#modal-periksa-diri">PERIKSA DIRI</button>
            <div class="container-fluid">
                    ...
                    <div class="card-body text-center">
                        ...
                        <div class="row">
                            <div class="col-12 text-center text-white" style="padding-bottom: 12px;">
                                <b >POSITIF</b> : <span id="positif">0</span>  |  <b>ODP</b> : <span id="odp">0</span>  |  <b>PDP</b> : <span id="pdp">0</span>  |  <b>Sembuh</b> : <span id="sembuh">0</span>  <br>  <b>Meninggal</b> : <span id="meninggal">0</span>  |  <b>RS</b> : <span id="rs">0</span>  |  <b>Posko</b> : <span id="posko">0</span>
                            </div>
                        </div>
	...
	@php
            $hari_ini = ['Demam suhu di atas 38 derajat Celcius', 'Sesak Nafas', 'Batuk kering tanpa radang tenggorokan', 'Lemah, letih, lesu', 'Radang Tenggorokan','Pilek'];
            $lingkungan = [
                [
                    'Riwayat perjalanan atau tinggal di kota berikut 30 hari yang lalu',
                    ['Jabodetabek', 'Medan', 'Bandung', 'Yogyakarta', 'Pontianak', 'Solo', 'Bali', 'Surabaya', 'Semarang', 'Tidak Pernah']
                ],
                [
                    'Riwayat perjalanan atau tinggal di negara berikut 30 hari yang lalu',
                    ['China', 'Italia', 'Malaysia', 'Amerika', 'Singapura', 'Inggris', 'Hongkong', 'Perancis', 'Korea', 'Jerman', 'Tidak Pernah']
                ],
                [
                    'Pernah memiliki kontak dengan orang yang dinyatakan positif COVID-19 atau PDP dalam 30 hari terakhir ?'
                ],
                [
                    'Pernah memiliki kontak dengan orang yang dinyatakan dalam pengawasan COVID-19 (ODP) dalam 30 hari terakhir ?'
                ],
                [
                    'Pernah mengunjungi pertemuan kantor atau lokasi yang ada orang positif COVID-19 ?'
                ],
                [
                    'Pernah mengunjungi fasilitas kesehatan COVID-19 dalam 30 hari terakhir ?'
                ],
                [
                    'Apakah lingkungan tempat tinggal anda terkena PSBB atau Zona Merah ?',
                    ['RT', 'RW', 'Kelurahan', 'Kecamatan', 'Kota/Kabupaten', 'Tidak Terkena']
                ]

            ];

            $nilai1 = [8,8,8,3,2,2];
            $nilai2 = [7,7,25,25,25,25,3];
        
        @endphp
	<!-- Modal -->
        <div class="modal fade" id="modal-periksa-diri" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
            <div class="modal-content">
                <div class="modal-header">
                <h5 class="modal-title text-white" id="exampleModalLabel">Periksa Diri</h5>
                <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
                </div>
                <div class="modal-body">
                    <form id="form-periksa-diri" action="#" method="post" style="color: black;">

                        @foreach ($hari_ini as $h)
                        <div style="background-color: white; margin-bottom: 16px; padding: 8px; border-radius: 8px;">
                            <p> {{ ($loop->iteration).' '.$h }} ?</p>
                            <div class="row">
                                <div class="col-6">
                                    <label class="radio-inline">
                                        <input type="radio" name="s{{ $loop->iteration }}" value="{{ $nilai1[$loop->iteration-1] }}" required> Ya
                                    </label>
                                </div>
                                <div class="col-6">
                                    <label class="radio-inline">
                                        <input type="radio" name="s{{ $loop->iteration }}" value="0" required> Tidak
                                    </label>
                                </div>
                            </div>
                        </div>
                        ...
                        @foreach ($lingkungan as $l)
                            ...
                            <div class="row">
                                ...
                                @if(!empty($l[1]))
                                    ...
                                    <div class="col-6">
                                        <label class="radio-inline">
                                            <input type="radio" name="s{{ $no }}" value="{{ ($o == 'Tidak Pernah' || $o == 'Tidak Terkena') ? 0 : $nilai2[$no-7] }}" required> {{ $o }}
                                        </label>
                                    </div>
                                    @endforeach
                                @else
                                        <div class="col-6">
                                            <label class="radio-inline">
                                                <input type="radio" name="s{{ $no }}" value="{{ $nilai2[$no-7] }}" required> Ya
                                            </label>
                                        </div>
                                        <div class="col-6">
                                            <label class="radio-inline">
                                                <input type="radio" name="s{{ $no }}" value="0" required> Tidak
                                            </label>
                                        </div>
                                @endif
                            </div>
	...
	<script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/js/bootstrap.min.js" integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl" crossorigin="anonymous"></script>
		...
		$('#form-periksa-diri').on('submit', function(e){
			var data = $(this).serializeArray();
			
			var nilai = 0, keterangan = "", badge = "", catatan = ""; 
			data.forEach((e) => {
				nilai += parseInt(e['value'])
			});
			...
			if(nilai <= 20)
			{
				keterangan = "NORMAL";
				badge = "success"
				catatan = "Jaga kesehatan, minum vitamin dan tetap dirumah saja";
			}
			else if(nilai > 20 && nilai <= 40)
			{
				keterangan = "SIAGA";
				badge = "info"; catatan = "Disarankan konsultasi ke dokter dan tetap dirumah saja";
			}
			else if(nilai > 40 && nilai <= 60)
			{
				keterangan = "WASPADA";
				badge = "warning"; catatan = "Segera ke Rumah Sakit atau ke dokter konsultasi SATGAS dan siap isolasi diri"
			}
			else if(nilai > 60)
			{
				keterangan = "BAHAYA";
				badge = "danger"; catatan = "Segera ke Rumah Sakit rujukan COVID-19 dan isolasi diri"
			}

			swal.fire({
				'title' : 'Hasil Perhitungan',
				'html' : `<h3>${nilai}</h3><p><span class="badge badge-${badge}">${keterangan}</span><br>${catatan}</p>`
			});
			e.preventDefault();
		});
    ...
    var semuaLayer = ['layer_positif','layer_odp','layer_pdp','layer_rs','layer_posko','layer_pd','layer_sembuh','layer_meninggal'];
    var copySemuaLayer = [];
...
map.on('locationerror', onLocationError);
    ...
    var jumlahSidoarjo = {'positif':0,'odp':0,'pdp':0,'rs':0,'posko':0,'pd':0, 'sembuh':0,'meninggal':0}
    var jenisKasus = ['positif','odp','pdp', 'rs', 'posko', 'pd', 'sembuh', 'meninggal']
    var firstLoad = true;
